...
class VideoStream:
    ...
    def __init__(self,resolution=(640,360),framerate=30):
        ...
        self.stream = cv2.VideoCapture(0)#'http://192.168.137.99:8080/video'
...
path="/media/external/"
full_path=path+time_string
video_writer = cv2.VideoWriter(full_path, fourcc, 30.0, (1280, 720))
#time.sleep(1)
fps = FPS().start()
start=time.time()
count_frame=0
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) and (classes[i]==0)):
            person_count=[num for num in scores if num >= 0.5] 
    ...
    frame_per_sec=1/(end-start)

    print("fps"+str(frame_per_sec))  
